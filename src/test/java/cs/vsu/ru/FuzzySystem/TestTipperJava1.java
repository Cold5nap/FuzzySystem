...
public class TestTipperJava1 {
	...
	static void animateFis(FIS fis) throws Exception {
		...
		// Set different values for 'food' and 'service'. Evaluate the system and show
		// variables
		...
		for (double service = 0.0, food = 1; service <= 10; service += 0.1) {
			...
			System.out.println(String.format("Service: %2.2f\tfood:%2.2f\t=> tip: %2.2f %%", service, food,
					fis.getVariable("tip").getValue()));
			jdf.repaint();
	...
	public static void main(String[] args) throws Exception {
		...
		MembershipFunction good = new MembershipFunctionTrapetzoidal(new Value(1), new Value(4), new Value(6),
				new Value(9));
		...
		RuleExpression antecedentOr = new RuleExpression(term1, term2, RuleConnectionMethodOrMax.get()); // Combine
																											// terms
																											// using
																											// connection
																											// methods:
