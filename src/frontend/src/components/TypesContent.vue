<template>
  <div id="types" class="col-3 bg-success bg-opacity-10 p-2">
    <p class="d-flex justify-content-between">
      <b>Типы</b>
      <i
        class="bi bi-plus-square text-success mx-1 h5"
    ...
    <ul class="list-group">
      <li
        class="list-group-item bg-success bg-opacity-10 p-1"
        ...
        <ul id="terms" class="list-group" v-if="type.terms.length > 0">
          ...
          >
            <div class="d-flex justify-content-between">
              ...
              <span>
                <i
                  class="bi bi-dash-square text-danger h5"
                  style="cursor: pointer"
                  @click="deleteTerm(type.terms, iTerm)"
                ></i>
                <i
                  class="bi bi-pencil-square text-info h5"
                  style="cursor: pointer"
                  @click="onEditTerm(term)"
                ></i>
              </span>
            </div>
            <div>
              ...
              <select
                ...
                v-if="editTerm === term"
                v-model="selectedTermFunc"
                @change="setSelectValues(term)"
              >
                <option v-for="(func, indexf) in termFunctions" :value="func" :key="indexf">
                  {{ func.name }}
                </option>
              </select>
            ...
            <span v-if="editTerm === term">
              <input
                class="form-control"
                v-for="(point, ipoint) in term.termFunction.points"
                type="number"
                v-model="term.termFunction.points[ipoint]"
                :key="ipoint"
                @change="updateParentTypes"
              />
            </span>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            >
              <option v-for="func in termFunctions" :key="func.name" :value="func">
                {{ func.name }}
              </option>
            </select>
...
export default {
  ...
  methods: {
    ...
    onEditTypeName(targetType) {
      if (this.editType != null) this.editType = null;
      else this.editType = targetType;
    },
    ...
    addTerm() {
      this.addType.terms.push({
        name: this.addTermName,
        termFunction: { name: this.addTermFunc.name, points: this.addTermPoints.slice() },
      });
      this.updateParentTypes();
    },
    onAddType() {
      this.types.push({ name: this.addTypeName, terms: [] });
      this.updateParentTypes();
    },
  },
  mounted() {},
